...
async function setSessionHttpOnlyCookie<T>(sessions: SessionCookie<T>[], iFrameEnabled: boolean = false) {
  ...
  if (sessions.length > 0) {
    const now = Date.now();
    const maxExpirationTs = Math.max(
      ...sessions.map((s) => (s.expirationTs ? Number(s.expirationTs) : 0))
    );
  ...
  return cookiesList.set({
    ...
    maxAge: maxAge ?? 30 * 24 * 60 * 60, // Default to 30 days if no sessions or no valid expiration
  });
}
